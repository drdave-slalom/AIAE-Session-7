...
terraform {
  ...
  #
  # CI supplies the state key at runtime via -backend-config.
  # Local validation can bypass the backend with `terraform init -backend=false`.
  backend "s3" {
...
# ---------------------------------------------------------------
# GitHub Actions configures short-lived AWS credentials through OIDC
